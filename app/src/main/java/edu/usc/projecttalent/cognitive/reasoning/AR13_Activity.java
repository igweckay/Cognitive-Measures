...
public class AR13_Activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_ar13_);
		Date start = new Date();
		String s = MyGlobalVariables.getTime();
		s+="ar13_start:"+start.toString()+";";
		MyGlobalVariables.setTime(s);
		ImageView myImage = (ImageView) findViewById(R.id.question_image);
	    myImage.setImageBitmap(ImageDecoder.decodeImage(getResources(),R.drawable.ar_4_main,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView1);
	    myImage.setImageBitmap(ImageDecoder.decodeImage(getResources(),R.drawable.ar_4_1,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView2);
	    myImage.setImageBitmap(ImageDecoder.decodeImage(getResources(),R.drawable.ar_4_2,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView3);
	    myImage.setImageBitmap(ImageDecoder.decodeImage(getResources(),R.drawable.ar_4_3,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView4);
	    myImage.setImageBitmap(ImageDecoder.decodeImage(getResources(),R.drawable.ar_4_4,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView5);
	    myImage.setImageBitmap(ImageDecoder.decodeImage(getResources(),R.drawable.ar_4_5,myImage.getLayoutParams().width,myImage.getLayoutParams().height));
		...
		quitButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				recordEndTime();
				Intent intent = new Intent(AR13_Activity.this, FinishActivity.class);
     ...
     button.setOnClickListener(new View.OnClickListener() {
         public void onClick(View v) {
         	...
         	if((click && count>=2)){
       		...
       		Intent intent=null;
			//create_textfile();
       		intent = new Intent(AR13_Activity.this, FinishActivity.class);
				startActivityForResult(intent,1);
         	}
         }
     });
	...
	{
		String s = MyGlobalVariables.getData();
		String ans= s.substring(s.indexOf("ar11:"));
		String[] l = new String[3];
		l = ans.split(";");count1=0;
		s=s.substring(0,s.indexOf("ar11:"));
		for(int i = 0;i<3;i++){
			if(i==0 && l[i].substring(l[i].indexOf(':')+1).equals("4")) {count1++;s+="ar11_score:1;";s+="ar11_ans:4;";}
			else if(i==1 && l[i].substring(l[i].indexOf(':')+1).equals("3")) {count1++;s+="ar12_score:1;";s+="ar12_ans:3;";}
			else if(i==2 && l[i].substring(l[i].indexOf(':')+1).equals("2")) {count1++;s+="ar13_score:1;";s+="ar13_ans:2;";}
			else
			{
				if(i==0){s+="ar11_score:0;";s+="ar11_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
				else if(i==1){s+="ar12_score:0;";s+="ar12_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
				else if(i==2){s+="ar13_score:0;";s+="ar13_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
			}
		}
		MyGlobalVariables.setData(s);
	}
		...
		new CountDownTimer(25000, 1000) {
			...
			public void onFinish() {
				if(!click & !exit ){
					//On finishing the 180 seconds, take the user to Quit/Resume activity.
					recordEndTime();
					Intent intent = new Intent(AR13_Activity.this, ARIntro_Activity.class);
					startActivityForResult(intent,1);
				}
			}
		}.start();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Log.d("AR13",  "onActivityResult - AR13");
        if (requestCode == 1) {
