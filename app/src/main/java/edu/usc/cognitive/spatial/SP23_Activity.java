...
public class SP23_Activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_sp23_);
		Date start = new Date();
		String s = MyGlobalVariables.getTime();
		s+="sp23_start:"+start.toString()+";";
		MyGlobalVariables.setTime(s);
		ImageView myImage = (ImageView) findViewById(R.id.question_image);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_5_main,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView1);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_5_1,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView2);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_5_2,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView3);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_5_3,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView4);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_5_4,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView5);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_5_5,myImage.getLayoutParams().width,myImage.getLayoutParams().height));
		...
		new CountDownTimer(60000, 1000) {
		     ...
		     public void onTick(long millisUntilFinished) {
		    	...
		    	else if(click && !empty)
		    	{
	            	...
	            	String ans= s.substring(s.indexOf("sp21:"));
	        		String[] l = new String[3];
	        		l = ans.split(";");count1=0;
	        		s=s.substring(0,s.indexOf("sp21:"));
	        		for(int i = 0;i<3;i++){
	        			if(i==0 && l[i].substring(l[i].indexOf(':')+1).equals("2")) {count1++;s+="sp21_score:1;";s+="sp21_ans:2;";}
	        			else if(i==1 && l[i].substring(l[i].indexOf(':')+1).equals("4")) {count1++;s+="sp22_score:1;";s+="sp22_ans:4;";}
	        			else if(i==2 && l[i].substring(l[i].indexOf(':')+1).equals("5")) {count1++;s+="sp23_score:1;";s+="sp23_ans:5;";}
	        			else
	        			{
	        				if(i==0){s+="sp21_score:0;";s+="sp21_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        				else if(i==1){s+="sp22_score:0;";s+="sp22_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        				else if(i==2){s+="sp23_score:0;";s+="sp23_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        			}
	        		}
