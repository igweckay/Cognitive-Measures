...
import edu.usc.cognitive.switching.SecSW_Activity;

public class SP43_Activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_sp43_);
		Date start = new Date();
		String s = MyGlobalVariables.getTime();
		s+="sp43_start:"+start.toString()+";";
		MyGlobalVariables.setTime(s);
		ImageView myImage = (ImageView) findViewById(R.id.question_image);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_15_main,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView1);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_15_1,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView2);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_15_2,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView3);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_15_3,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView4);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_15_4,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView5);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_15_5,myImage.getLayoutParams().width,myImage.getLayoutParams().height));
		...
		new CountDownTimer(60000, 1000) {
		     ...
		     public void onTick(long millisUntilFinished) {
		    	...
		    	else if(click && !empty)
		    	{
	            	...
	            	String ans= s.substring(s.indexOf("sp41:"));
	        		String[] l = new String[3];
	        		l = ans.split(";");count1=0;
	        		s=s.substring(0,s.indexOf("sp41:"));
	        		for(int i = 0;i<3;i++){
	        			if(i==0 && l[i].substring(l[i].indexOf(':')+1).equals("1")) {count1++;s+="sp41_score:1;";s+="sp41_ans:1;";}
	        			else if(i==1 && l[i].substring(l[i].indexOf(':')+1).equals("4")) {count1++;s+="sp42_score:1;";s+="sp42_ans:4;";}
	        			else if(i==2 && l[i].substring(l[i].indexOf(':')+1).equals("3")) {count1++;s+="sp43_score:1;";s+="sp43_ans:3;";}
	        			else
	        			{
	        				if(i==0){s+="sp41_score:0;";s+="sp41_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        				else if(i==1){s+="sp42_score:0;";s+="sp42_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        				else if(i==2){s+="sp43_score:0;";s+="sp43_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        			}
	        		}
