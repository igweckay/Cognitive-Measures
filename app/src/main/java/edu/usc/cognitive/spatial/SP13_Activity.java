...
public class SP13_Activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_sp13_);
		Date start = new Date();
		String s = MyGlobalVariables.getTime();
		s+="sp13_start:"+start.toString()+";";
		MyGlobalVariables.setTime(s);
		ImageView myImage = (ImageView) findViewById(R.id.question_image);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_2_main,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView1);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_2_1,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView2);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_2_2,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView3);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_2_3,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView4);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_2_4,myImage.getLayoutParams().width,myImage.getLayoutParams().height));

	    myImage = (ImageView) findViewById(R.id.imageView5);
	    myImage.setImageBitmap(decodeImage(getResources(),R.drawable.sp_2_5,myImage.getLayoutParams().width,myImage.getLayoutParams().height));
		...
		new CountDownTimer(60000, 1000) {
		     ...
		     public void onTick(long millisUntilFinished) {
		    	...
		    	else if(click && !empty)
		    	{
	            	...
	            	String ans= s.substring(s.indexOf("sp11:"));
	        		String[] l = new String[3];
	        		l = ans.split(";");count1=0;
	        		s=s.substring(0,s.indexOf("sp11:"));
	        		for(int i = 0;i<3;i++){
	        			if(i==0 && l[i].substring(l[i].indexOf(':')+1).equals("1")) {count1++;s+="sp11_score:1;";s+="sp11_ans:1;";}
	        			else if(i==1 && l[i].substring(l[i].indexOf(':')+1).equals("2")) {count1++;s+="sp12_score:1;";s+="sp12_ans:2;";}
	        			else if(i==2 && l[i].substring(l[i].indexOf(':')+1).equals("1")) {count1++;s+="sp13_score:1;";s+="sp13_ans:1;";}
	        			else
	        			{
	        				if(i==0){s+="sp11_score:0;";s+="sp11_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        				else if(i==1){s+="sp12_score:0;";s+="sp12_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        				else if(i==2){s+="sp13_score:0;";s+="sp13_ans:"+l[i].substring(l[i].indexOf(':')+1)+";";}
	        			}
	        		}
